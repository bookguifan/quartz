...
**CNN组成部分**:
1. 卷积层conv ->负责从输入图像中自动提取局部的特征,比如纹理,边缘.（只做加权和，因此需要接激活函数，一般使用relu）
1.1 激活函数层relu -> 引入非线性能力-> 让模型学习到更复杂的模式
2. 池化层pool ->用来降低特征图的空间维度(降维),减少计算量和参数数量
3. 全连接层fc -> 位于网络的末端-> 结果输出 -> 最终的分类决策
**CNN 的典型应用场景**：
...
padding:在输入的图像周围补一圈(0)
1. 控制最终特征图的尺寸大小(防止图像越缩越小)
2. 保护边缘信息->让边缘信息更多的被利用到
