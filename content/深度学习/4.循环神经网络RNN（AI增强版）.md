...
> - **文本生成案例**：基于周杰伦歌词数据集构建 RNN 模型，通过**词嵌入 + RNN + 全连接**三层结构实现==字符级文本生成==。
...
# 分词后的词语 = 词表
# 每个词都使用一个向量来表示 = 词向量
# 输入的词数量由源文本分词后的数量决定，输出的每个词向量的维度是一个超参数，由开发者决定
# 所有单词组成的词向量 = 词向量矩阵
```
### 进阶用法与原理
...
- $W_{ih}$：输入到隐藏层的权重矩阵
- $W_{hh}$：隐藏层到隐藏层的权重矩阵
- $b_{ih}, b_{hh}$：偏置项,表示输入数据和表示输入数据的偏置
- $\tanh$：激活函数，将输出压缩到 $[-1, 1]$
