...
1. **概念索引**：从每篇增强版笔记的"1️⃣ 完整知识库"模块提取核心概念，按逻辑分类组织，每个概念附一句话定义和来源链接。
   - **提取规则**：扫描 `## 主题名 🔸 核心` 和 `## 主题名 🔹 基础` 格式的一级主题标题
   - **子主题提取**：扫描 `### 定义与本质`、`### 基础用法` 等二级标题下的粗体术语 `**术语**`
   - **来源标注**：每个概念必须标注来源笔记，格式为 `[[笔记名（AI增强版）]]`
...
6. **索引完整性**：生成/更新时必须扫描全部增强版笔记，确保概念索引区覆盖所有笔记；新增笔记后必须补充对应条目。
   - **新增笔记检测**：对比上次生成时的笔记列表，发现新增笔记后立即补充对应概念索引

#### 1.5 知识地图完整性检查清单

输出知识地图前，必须逐项确认：

- [ ] 概念索引区是否覆盖全部增强版笔记的核心概念？
- [ ] 新增笔记后，是否补充了对应的概念索引条目？
- [ ] 学习路径区是否包含新增笔记的链接？
- [ ] 评估指标速查是否与算法笔记中的指标一致？
- [ ] sklearn API 速查中的类名是否与代码示例中的 import 一致？
...
- **文件编码**：`content.json` 必须以 **UTF-8 with BOM** 编码保存，确保 Windows 环境正确识别
- **字体配置**：Theme 中必须配置中文字体回退栈，避免中文显示为系统默认字体（宋体）
- **子节点 class 检查**：生成后必须扫描 `content.json`，确保除 `rootTopic` 外，任何子节点均无 `"class": "topic"`
...
```json
{
  "subTopic": {
    "id": "uuid", "properties": {
      "fo:font-family": "Droid Serif, Microsoft YaHei, PingFang SC, Noto Sans CJK SC, sans-serif",
      "fo:font-size": "14pt",
